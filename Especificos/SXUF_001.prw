...
User Function SXUF_001()
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML006" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Momento de validação do PC"
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML007" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Momento de validação dos produtos"
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML008" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Cadastra cliente/fornecedor na importação"
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML016" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Condição pagamento padrao NFE"
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML025" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Maior código de fornecedor na inclusão automática"
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML026" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Maior código de cliente na inclusão automática"
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML029" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Numeração dos lotes de envio de eventos"
			SX6->X6_CONTEUD := SX6->X6_CONTSPA := SX6->X6_CONTENG := "000000000000000"
			SX6->X6_PROPRI  := 'U'
		SX6->( MsUnlock() )
	EndIf
	...
	If !SX6->( DbSeek( Space( Len( xFilial("SD1") ) ) + "SOL_XML030" ) )
		RecLock("SX6", .T.)
			...
			SX6->X6_TIPO    := "C"
			SX6->X6_DESCRIC := SX6->X6_DSCSPA  := SX6->X6_DSCENG  := "Horario de verão ativo"
			SX6->X6_CONTEUD := SX6->X6_CONTSPA := SX6->X6_CONTENG := "S"
			SX6->X6_PROPRI  := 'U'
		SX6->( MsUnlock() )
	EndIf
	...
	If _lRet
		SXSF_001()
	Else
		MsgAlert("Não foi possível acessar, verifique com o administrador!")
	EndIf
	...
	ElseIf cTipDoc == "CTE"
		...
		If Len( aNfRef ) > 0
			...
			If cTpFrete == "C"
				...
				//Adicionado por diognes barazzutti
				//Busca a natureza no campo  SA2_NATUREZ, se não houver nenhum valor lá, considera o antigo valor de 40113
			...
			Else
				...
				If lMostra .Or. ( Empty( cClaVlr ) .Or. Empty( cItCont ) .Or. Empty( cCenCus ) )
					...
					oDlgCont := MSDialog():New( 157,338,396,553,"Informações Contábeis",,,.F.,,,,,,.T.,,,.T. )
					...
					oGetIC   := TGet():New( 048,008,{|u| If(Pcount()>0,cItCont:=u,cItCont)},oDlgCont,060,008,PesqPict("SD1","D1_ITEMCTA"),{|| Vazio().Or.Ctb105Item() },CLR_BLACK,CLR_WHITE,,,,.T.,"",,,.F.,.F.,,.F.,.F.,"CTD","cItCont",,,,.F.,.F.,,"Item Contábil  ",1)
...
// Função que faz a replicação da TES informada no primeiro item
