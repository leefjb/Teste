...
User Function JMHF080( aTransf, lEstorno,nlinha )
	...
	Else
		...
		dbSetOrder(1)
		DBGOBOTTOM()
		cDoc := Soma1(IIF(EMPTY(SD3->D3_DOC), "000000001", SD3->D3_DOC)) //NextNumero("SD3",2,"D3_DOC",.T.)
		...
		Else
			...
			For nItem := 1 To Len(aTransf)
				...
				If SB1->B1_RASTRO = "L" //L=Lote
					...
					For nP := 1 To aTransf[nItem,3]
						...
						//Novos Campos Obrigatorios no ExecAuto MATA261 - v12.1.17
						aAdd(aItem,"")					//D3_IDDCF
			...
			If Len(aAuto) > 1
			    ...
			    /*		      
				Prod.Orig. D3_COD      C        15       0 OK
				Desc.Orig. D3_DESCRI   C       120       0 OK
				UM Orig.   D3_UM       C         2       0 OK
				Armazem Or D3_LOCAL    C         2       0 OK
				Endereco O D3_LOCALIZ  C        15       0 OK
				Prod.Desti D3_COD      C        15       0 OK
				Desc.Desti D3_DESCRI   C       120       0 OK
				UM Destino D3_UM       C         2       0 OK
				Armazem De D3_LOCAL    C         2       0 OK
				Endereco D D3_LOCALIZ  C        15       0 OK  
				Numero Ser D3_NUMSERI  C        20       0 OK
				Lote       D3_LOTECTL  C        10       0 OK
				Sub-Lote   D3_NUMLOTE  C         6       0 OK
				Validade   D3_DTVALID  D         8       0 OK
				Potencia   D3_POTENCI  N         6       2 OK
				Quantidade D3_QUANT    N        11       2 OK
				Qt 2aUM    D3_QTSEGUM  N        12       2 OK
				Estornado  D3_ESTORNO  C         1       0 OK
				Sequencia  D3_NUMSEQ   C         6       0 OK  
				Lote Desti D3_LOTECTL  C        10       0 OK
				Validade D D3_DTVALID  D         8       0 OK
				Item Grade D3_ITEMGRD  C         3       0 OK
				Id DCF     D3_IDDCF    C         6       0
				Observação D3_OBSERVA  C        30       0
				Lote Orige D3_LTCTORI  C        10       0 OK
				DT VaLote  D3_DTVORI   D         8       0 OK
				Doc Origem D3_DOCORI   C         9       0 OK
				Serie Orig D3_SERORI   C         3       0 OK
				Doc>origem D3_FORORI   C         6       0 OK
				Loja Orige D3_LOJORI   C         2       0 OK
				Orig Lanct D3_LANORI   C         2       0 OK
			    */
...
// URGENTE - CORRECAO LOTE SC

Static Function CNTGNC(_aAuto)

	Local nI := 0
	
	For nI := 1 To Len(_aAuto)
		If nI = 1          			
			dbSelectArea("SB8")
			dbSetOrder(3)
			dbSeek( "01" + _aAuto[2, 01] + _aAuto[2, 04] + _aAuto[2, 12] )
		    dB8Data := SB8->B8_DATA
		    dB8DFab := SB8->B8_DFABRIC
		...
		Else
			...
			SB8->B8_FILIAL  := "01"
			SB8->B8_QTDORI  := 1
			SB8->B8_PRODUTO := _aAuto[nI, 1]
			SB8->B8_LOCAL   := "01"
			SB8->B8_DATA    :=	dB8Data
			SB8->B8_DTVALID := _aAuto[nI, 14]			
			SB8->B8_SALDO   := 1
			SB8->B8_EMPENHO := 0
			SB8->B8_ORIGLAN := _aAuto[nI, 31]			
			SB8->B8_LOTEFOR := _aAuto[nI, 12]
			SB8->B8_LOTECTL := _aAuto[nI, 20]
			SB8->B8_DOC     := _aAuto[nI, 27]
			SB8->B8_SERIE   := _aAuto[nI, 28]
			SB8->B8_CLIFOR  := _aAuto[nI, 29]
			SB8->B8_LOJA    := _aAuto[nI, 30]  
			SB8->B8_DFABRIC := dB8DFab
			SB8->B8_LOTEUNI := "S"
			MsUnLock()
	    EndIf
	Next nI                           
