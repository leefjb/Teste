	...
	Else
		...
		If Pergunte(cPerg,.T.)
			...
			While (cArea)->( !Eof() )
				...
				// SE ESTA PREENCHIDO, CONSIDERA A QUANTIDADE DEFINIDA. SENAO, UTILIZA TODO O SALDO
				If !Empty(MV_PAR07)
					nSaldoB8 := MV_PAR07
				Else
					nSaldoB8 := (cArea)->B8_SALDO
				EndIf
				
				For nZ := 1 To nSaldoB8 //(cArea)->B8_SALDO
...
Static Function A030CriaSx(cPerg)
	...
	aAdd(aP,{	"Quantidade ?",	"N",	TamSX3("B8_SALDO")[1],		    2,		"G",				"",	 	"",		   "",		"",		"",		"",		""})
	...
	aAdd(aHelp, {"Quantidade a Estourar"})	
