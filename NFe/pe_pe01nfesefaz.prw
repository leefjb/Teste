...
Private _cETIQ	:= "" //jomhedica - etiquetas ACD   
...
//				cMensCli += Alltrim(SF4->F4_MENS)+"/p/" Rodrigo Carvalho Solutio, correção para remover a impressão dos caracteres /p/ 13/12/2016
			...
			/////////ACD - jomhedica  /////somente para o ACD - empresa 06 - informando as etiquetas
			If ( (cEmpAnt == "06" .or. cEmpAnt == "01")  .and. ! Empty(SD2->D2_LOTECTL) )
				_cEtq1 := u_EtiqNF(SD2->D2_DOC,SD2->D2_SERIE,SD2->D2_CLIENTE,SD2->D2_LOJA,SD2->D2_COD,SD2->D2_ITEM )
				If (Alltrim(_cEtq1) <> "" )
					__cInfoProd += Padr( _cEtq1 + ' VALIDADE: '+SUBSTR(DTOC(SD2->D2_DTVALID), 4),35)
				EndIf
			EndIf   
			
			/////////ACD - jomhedica  /////somente para o ACD - empresa 06 - etiqueta A.F. - impressao C6_XETIQ
			If ( (cEmpAnt == "06" .or. cEmpAnt == "01") .and. ! Empty(SD2->D2_LOTECTL) )
				_cEtq2 := u_EtiqAF(SD2->D2_DOC,SD2->D2_SERIE,SD2->D2_CLIENTE,SD2->D2_LOJA,SD2->D2_COD,StrZero(val(SD2->D2_ITEM),2) ,SD2->D2_PEDIDO, SD2->D2_ITEMPV )
				If (Alltrim(_cEtq2) <> "" )
					__cInfoProd += Padr( _cEtq2 + ' VALIDADE: '+SUBSTR(DTOC(SD2->D2_DTVALID), 4),35)
				EndIf				
			EndIf
			
			
			
			/*########################## Leef  - 15/10/2019 - Assis ##############################
			  ##  Ajuste para impressão do Número de Serie proveniente do Spark                 ##
			  ####################################################################################
			*/
			if !Empty(SD2->D2_SPKSER)
				__cInfoProd += Padr('N.SERIE : '+SD2->D2_SPKSER,35)
			else 	
			  If ! Empty(SD2->D2_NUMSERI)
			  	__cInfoProd += Padr(' N.SERIE : '+Alltrim(SD2->D2_NUMSERI),35)
			  EndIf          
			Endif
			/*############################ Leef  - FIM ##########################################*/
     			// numero do lote e validade
     			If  ! Empty(SD2->D2_LOTECTL)   /// Empty(SD2->D2_NUMSERI) .and.
     				...
     				If ( Alltrim(_cEtq1) == "" .and. Alltrim(_cEtq2) == "" ) 
     					__cInfoProd += Padr('LOTE : '+SD2->D2_LOTECTL + ' VAL:'+SUBSTR(DTOC(SD2->D2_DTVALID), 4),35)
     				Else
     					__cInfoProd += Padr('LOTE: '+SD2->D2_LOTECTL,35)
     				EndIf  				
     			EndIf                                                                                 
					// lote do fornecedor 
					...
					// Retirado o campo do Lotedo fornecedor por Assis em 07/11/19-17:01
			...
			//			__cInfoProd += Padr('LOTE FORNECEDOR: ' + SB8->B8_LOTEFOR ,35)
			//		EndIf
			...
			If ! Empty(SB1->B1_RMS)
				...
				__cInfoProd += Padr('RMS: '+ SB1->B1_RMS,35)  
			EndIf
...
//				cMensCli += Alltrim(SF4->F4_MENS)+"/p/"  Rodrigo Carvalho Solutio, correção para remover a impressão dos caracteres /p/ 13/12/2016 
		...
		ElseIf	SD1->D1_NFIMP == '2'   // venda com troca de produto
			// adiciona os dados de informacao adicional do produto				
			aadd(aProdInfAd,{aProd[nP,1],;
							 SD1->D1_KITPAI,;
							 SD1->D1_COD,;
