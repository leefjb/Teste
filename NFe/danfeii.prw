...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
	...
	//RAFAEL SCHEIBLER - 25/10/2017
	// Private aBoletos   := Array(0)  	// especifico - impressao de boletos   Retira por Assis em 160819-1646 - Esta
	//                                     definido no PE_M460FIM                 
	...
	If nTipo <> 1
		...
		oDanfe:lServer := Iif ( (ProcName(1) <> "XSPEDDANFE"), oSetup:GetProperty(PD_DESTINATION)==AMB_SERVER, .F. )
		// ----------------------------------------------
		// Define saida de impressão
		// ----------------------------------------------
		If (ProcName(1) <> "XSPEDDANFE") .and. oSetup:GetProperty(PD_PRINTTYPE) == IMP_SPOOL  //.or.  (ProcName(1)== "XSPEDDANFE") 
			oDanfe:nDevice := IMP_SPOOL
			// ----------------------------------------------
			// Salva impressora selecionada
			// ----------------------------------------------
			fwWriteProfString(GetPrinterSession(),"DEFAULT", oSetup:aOptions[PD_VALUETYPE], .T.)
			If (ProcName(1)== "XSPEDDANFE") 
				//oDanfe:CSESSION := "PRINTER_SRVRSPOAAPP04"
				//oDanfe:cprinter	:=  "Microsoft Print to PDF" //"\\192.168.0.5\IMP-Logistica"
				//oDanfe:CPATCHPRINT := "\\SPOOL\\"
			Else
				oDanfe:cPrinter := oSetup:aOptions[PD_VALUETYPE]
			EndIf
			
		ElseIf (ProcName(1) <>  "XSPEDDANFE") .and. oSetup:GetProperty(PD_PRINTTYPE) == IMP_PDF
		...
		Else
			...
			If lVerPerg .and. ProcName(1) <> "XSPEDDANFE" ///////////////////
				lPergunte := Pergunte("NFSIGW",.T.)
			ElseIf (ProcName(1)== "XSPEDDANFE") //////////////////////////
				lPergunte := .T.
				Pergunte("NFSIGW",.F.)
				MV_PAR01 := TMP->F2_DOC
				MV_PAR02 := TMP->F2_DOC
				...
				MV_PAR05 := 2	//[Frente e Verso] Nao
				MV_PAR06 := 2	//[DANFE simplificado] Nao
				MV_PAR07 := StoD(TMP->F2_EMISSAO) -10
				MV_PAR08 := StoD(TMP->F2_EMISSAO) +10
			EndIf
		EndIf
		...
		If lExistNFe
			...
			If (ProcName(1) == "XSPEDDANFE")
				//File2Printer(oDanfe:cPathPDF + cFilePrint +".pdf" ,"PDF")
				//oDanfe:CSPOOLLOCAL := "c:\rel\"
				//oDanfe:CPATHPRINT := "\\SPOOL\\"
				//oDanfe:CPRINTER := ""		
			...
			//#16046 Incluído em 14/12/2016, pois não estava no fonte. Mauro - Solutio.
			// especifico - impressao de boletos
			// Acertados os parâmetros com numeração adequada em relação ao P11 - RAFAEL SCHEIBLER - SOLUTIOIT

        //    Assis em 160819 - 1648 - Rotina foi para PE_M460FIM
		//   	If MV_PAR10 = 1
		//   		lPrevBol := (MV_PAR11 = 1)
		//   		cMens1   := MV_PAR12
		//   		cMens2   := MV_PAR13
		...
		//   
		//   				u_JHR100Bo(aBoletos[nStart,2], aBoletos[nStart,1], 'NF ',, lPrevBol, {cMens1, cMens2},, _cBancoCobr, _cAgenciaCobr, _cContaCobr, _cSubCtaCobr, {aBoletos[nStart,3][1], aBoletos[nStart,3][2], ''})
		//   
		//   			Next
		//   		//EndIf
		//   	EndIf
...
If !lImpDir
	...
	#IFDEF TOP
		...
		ElseIf MV_PAR04==2
			...
			Else
				cSerie := Padr(MV_PAR03,TamSx3("F3_SERIE")[1])
				cWhere := "%SubString(SF3.F3_CFO,1,1) >= '5' AND SF3.F3_SERIE = '"+ cSerie + "' AND SF3.F3_ESPECIE = 'SPED'" + iif(!empty(MV_PAR14),"AND SF3.F3_CLIEFOR IN " + FormatIn(AllTrim(mv_par14),";"),"")
			Endif
		Else
...
// Assis 160819-16:18 	// especifico - tratamento de boletos e etiquetas
// Assis 160819-16:18 	// somente se o cliente pedir boleto - implementado em 07/05/2010
// Assis 160819-16:18 	If MV_PAR04 == 2
// Assis 160819-16:18 	
// Assis 160819-16:18 		_cETIQ	:= "" //jomhedica
// Assis 160819-16:18 
// Assis 160819-16:18 		dbSelectArea("SF2")
// Assis 160819-16:18 		dbSetOrder(1)
// Assis 160819-16:18 		If MsSeek(xFilial("SF2")+aNota[05]+aNota[04]+aNota[06]+aNota[07])
// Assis 160819-16:18 			SA1->( dbSeek(xFilial('SA1')+SF2->F2_CLIENTE+SF2->F2_LOJA) )
// Assis 160819-16:18 			SD2->( dbSetOrder(3) )
// Assis 160819-16:18 			SD2->( dbSeek(xFilial('SD2')+SF2->F2_DOC+SF2->F2_SERIE) )
// Assis 160819-16:18 			SD2->( dbSetOrder(1) )
// Assis 160819-16:18 			SC5->( dbSeek(xFilial('SC5')+SD2->D2_PEDIDO) )
// Assis 160819-16:18 			If SA1->A1_BOLCLI == '1'
// Assis 160819-16:18 				If SC5->C5_TPDESC == '1'
// Assis 160819-16:18 					aDesc := {SC5->C5_DESCOM, 0, ''}
// Assis 160819-16:18 				Else
// Assis 160819-16:18 					aDesc := {0, SC5->C5_DESCOM, ''}
// Assis 160819-16:18 				EndIf
// Assis 160819-16:18 				If SZC->( dbSeek(xFilial('SZC')+SA1->A1_CLASSE) )
// Assis 160819-16:18 					If SZC->ZC_PRNBOL == '1'
// Assis 160819-16:18 						Aadd(aBoletos, {SF2->F2_DOC, SF2->F2_SERIE, aDesc, {SZC->ZC_CODIGO, SZC->ZC_AGENCIA, SZC->ZC_CONTA, SZC->ZC_SUBCTA}})
// Assis 160819-16:18 					EndIf
// Assis 160819-16:18 				EndIf
// Assis 160819-16:18 			EndIf
// Assis 160819-16:18 		EndIf
...
For nZ := 1 To nLenDet
	...
	If (Type("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. Type("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav .Or. lImpInfAd )
		...
		/////customizacao: tratamento etiquetas - Jomhedica - auxilar descricao produto na Danfe
		///////////////////////////////// 
		//If (MV_PAR04 == 2) //////documento de saida
			//cAux += u_EtiqNF(SF2->F2_DOC,SF2->F2_SERIE,SF2->F2_CLIENTE,SF2->F2_LOJA,oDet[nX]:_Prod:_CPROD:TEXT,StrZero(val(oDet[nX]:_NITEM:TEXT),2))
		//EndIf
		
		If (cEmpAnt == "06" .and. MV_PAR04 == 2)//.and. ! Empty(SD2->D2_LOTECTL) )

			_aAreaAF := GetArea()
			dbSelectArea("SD2")
			dbSetOrder(3)//D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_COD+D2_ITEM
			dbSeek(xFilial("SD2") + SF2->F2_DOC + SF2->F2_SERIE + SF2->F2_CLIENTE + SF2->F2_LOJA + Padr( oDet[nX]:_Prod:_CPROD:TEXT , TamSx3("D2_COD")[1] )  + Padr( StrZero(val(oDet[nX]:_NITEM:TEXT),2) , TamSx3("D2_ITEM")[1] )  )                                                                                                      
		
			If Found()
			
				cAux += u_EtiqAF(SD2->D2_DOC,SD2->D2_SERIE,SD2->D2_CLIENTE,SD2->D2_LOJA,SD2->D2_COD,StrZero(val(SD2->D2_ITEM),2) ,SD2->D2_PEDIDO, SD2->D2_ITEMPV )
