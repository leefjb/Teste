...
User Function MT010INC( lFlag )

	Local aArea    := GetArea()

	Default lFlag  := .t.     // INCLUSAO

	lUsaSpark := SuperGETMV("MV_SPARK",.F.)
	if lUsaSpark
		FwMsgRun(,{|| u_IncluiZX4("SB1",SB1->B1_FILIAL+SB1->B1_COD,SB1->(Recno())) },"Integração SPARK","Agendando integração WMS.")
	endif
