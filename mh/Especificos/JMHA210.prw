...
Static Function GeraComis()
	...
	*/
	...
	cQuery1  += "FROM " + RetSqlName("SE3") +" SE3, " 
	cQuery1  += "     " + RetSqlName("SD2") +" SD2, " 
	cQuery1  += "     " + RetSqlName("SB1") +" SB1, " 
	cQuery1  += "     " + RetSqlName("SF4") +" SF4, " 
	cQuery1  += "     " + RetSqlName("SA1") +" SA1, " 
	cQuery1  += "     " + RetSqlName("SE1") +" SE1 " 
	...
	cQuery2  += "FROM " + RetSqlName("SE3") +" SE3, "
	cQuery2  += "     " + RetSqlName("SE5") +" SE5, "	 
	cQuery2  += "     " + RetSqlName("SD2") +" SD2, " 
	cQuery2  += "     " + RetSqlName("SB1") +" SB1, " 
	cQuery2  += "     " + RetSqlName("SF4") +" SF4, " 
	cQuery2  += "     " + RetSqlName("SA1") +" SA1, " 
	cQuery2  += "     " + RetSqlName("SE1") +" SE1 " 
	...
	cQuery2  +=                          " FROM " + RetSqlName("SE1") + " SSE1 "
	...
	cQuery3  += "FROM " + RetSqlName("SE3") +" SE3, "
	cQuery3  += "     " + RetSqlName("SE5") +" SE5, "	 
	cQuery3  += "     " + RetSqlName("SD2") +" SD2, " 
	cQuery3  += "     " + RetSqlName("SB1") +" SB1, " 
	cQuery3  += "     " + RetSqlName("SF4") +" SF4, " 
	cQuery3  += "     " + RetSqlName("SA1") +" SA1, " 
	cQuery3  += "     " + RetSqlName("SE1") +" SE1 " 
	...
	cQuery4  += "FROM " + RetSqlName("SE3") +" SE3, "
	cQuery4  += "     " + RetSqlName("SE5") +" SE5, "	 
	cQuery4  += "     " + RetSqlName("SD2") +" SD2, " 
	cQuery4  += "     " + RetSqlName("SB1") +" SB1, " 
	cQuery4  += "     " + RetSqlName("SF4") +" SF4, " 
	cQuery4  += "     " + RetSqlName("SA1") +" SA1, " 
	cQuery4  += "     " + RetSqlName("SE1") +" SE1 " 
	...
	cQuery4  += "AND   E5_FATURA       IN ( SELECT DISTINCT E5_NUMERO FROM " + RetSqlName("SE5")+" SSE5 "
	cQuery4  += "			               WHERE SSE5.E5_DOCUMEN  IN ( SELECT DISTINCT E1_NUMLIQ   "
	cQuery4  += "		                                              FROM " + RetSqlName("SE1")+" SSE1 "
	...
	For _nV := 1 To Len( aVen )
		...
		For _nC := 1 To Len( aNfs )
			...
			For _nI := 1 To Len( aIte )
				...
				RecLock("SZE",.T.)
					ZE_FILIAL  := xFilial("SZE")  // Filial
					ZE_VEND    := aVen[ _nV, 01 ] // Vendedor
					ZE_GRUPO   := aIte[ _nI, 09 ] // Grupo de Produto
					ZE_CODFAIX := aIte[ _nI, 10 ] // Código da Faixa
					ZE_TPFAIXA := aIte[ _nI, 04 ] // Tipo de Cálculo (Faixa de Desconto ou Faixa de Faturamento)
					ZE_DESCF   := aNfs[ _nC, 14 ] // Desconto Financeiro
					ZE_QTDPARC := aIte[ _nI, 11 ] // Quantidade de parcelas da condição
					ZE_CLIENTE := aNfs[ _nC, 08 ] // Cliente
					ZE_LOJA    := aNfs[ _nC, 09 ] // Loja
					ZE_E3PREFI := aNfs[ _nC, 15 ] // Prefixo do SE3
					ZE_E3NUM   := aNfs[ _nC, 16 ] // Número do SE3
					ZE_E3PARC  := aNfs[ _nC, 17 ] // Parcela do SE3
					ZE_E3TIPO  := aNfs[ _nC, 18 ] // Tipo do SE3
					ZE_E3SEQ   := aNfs[ _nC, 10 ] // Sequencia do SE3
					ZE_E1PREFI := aNfs[ _nC, 01 ] // Prefixo do SE1
					ZE_E1NUM   := aNfs[ _nC, 02 ] // Número do SE1
					ZE_E1PARC  := aNfs[ _nC, 03 ] // Parcela do SE1
					ZE_E1TIPO  := aNfs[ _nC, 04 ] // Tipo do SE1
					ZE_BASENF  := aNfs[ _nC, 05 ] // Base da faixa nesta nota
					ZE_COMISNF := aNfs[ _nC, 06 ] // Comissão da faixa nesta nota
					ZE_PERCNF  := aNfs[ _nC, 07 ] // Percentual de comissão da faixa nesta nota
					ZE_BASEITE := aIte[ _nI, 06 ] // Base do item
					ZE_PERCITE := aIte[ _nI, 07 ] // Percentual de comissão do item
					ZE_COMIITE := aIte[ _nI, 08 ] // Comissão do item
					ZE_DEMISE1 := aNfs[ _nC, 13 ] // Data de Emissão do SE1 (mesma da nota fiscal)
					ZE_DEMISE3 := aNfs[ _nC, 11 ] // Data de emissão do SE3 (mesma da baixa do título no SE1)
					ZE_TOTALD2 := aIte[ _nI, 02 ] // Total do item na venda
					ZE_PRODD2  := aIte[ _nI, 01 ] // Produto do item da nota
					ZE_PVD2    := aIte[ _nI, 03 ] // Pedido de venda
					ZE_DESCCOM := aIte[ _nI, 05 ] // Percentual de desconto comercial do item (quando cálculo for por faixa de desconto)
					ZE_E1VENCR := aNfs[ _nC, 23 ] // Vencimento Real do título
					ZE_E1VALOR := aNfs[ _nC, 24 ] // Valor original do título
					ZE_D2CUST1 := aIte[ _nI, 12 ] // Custo unitário do item
					ZE_D2ICMS  := aIte[ _nI, 12 ] // Icms do item
					ZE_D2IPI   := aIte[ _nI, 12 ] // Ipi do item
		        SZE->( MsUnLock() )
			Next
		Next
	Next
