...
Static Function F031Confir(aBrowse, cOrigem)
	...
	ElseIf cOrigem = "LOJ"
		...
		For nItem := 1 To Len(aBrowse)
			...
			If dbSeek( xFilial("SL1") + aBrowse[nItem][NPOS_C9_PEDIDO] )
			
				RecLock("SL1", .F.)
					SL1->L1_USRCON := CUSERNAME
					SL1->L1_PVSTAT := CONFERIDO 	
				MsUnLock()
				
				dbSelectArea("SL2")
				dbSetOrder(1)
		
				If dbSeek( xFilial("SL2") + aBrowse[nItem][NPOS_C9_PEDIDO] + aBrowse[nItem][NPOS_C9_ITEM] )
	
					RecLock("SL2", .F.)
						SL2->L2_QTDCONF := aBrowse[nItem][NPOS_QTDCONF] 	
					MsUnLock()
		...
		Else
			...
			If aBrowse[nPosItem,NPOS_QTDCONF] < aBrowse[nPosItem,NPOS_QTDVEND]
				...
				If aBrowse[nPosItem,NPOS_LOTE][nPosLote][NLOT_QTCONF] = aBrowse[nPosItem,NPOS_LOTE][nPosLote][NLOT_QTLIB]
					...
					cMsgLin1 := cProduto +" Lote: "+ cLoteCtl			//-- Produto + Lote
		 			cMsgLin2 := aBrowse[nPosItem,NPOS_B1_DESC]		//-- Descricao Produto
		 			cMsgLin3 := "Produto + Lote já conferido"			//-- Mensagem
		 			cMsgStat := StrZero(aBrowse[nPosItem,NPOS_QTDCONF],IIf( Len(cValToChar(aBrowse[nPosItem,NPOS_QTDVEND])) <= 6, 3, 6 ) )+" / "+;
		 						StrZero(aBrowse[nPosItem,NPOS_QTDVEND],IIf( Len(cValToChar(aBrowse[nPosItem,NPOS_QTDVEND])) <= 6, 3, 6 ))	//-- Status quantidades Vendida X Conferida
...
Static Function F031Produt(cPedido, cOrigem, cMvFluxo, cNf, cSerie, cCliente, cLoja)  
	...
	If cOrigem = "FAT"
		...
		If cMvFluxo = "1"
			...
			cQuery := " SELECT 	D2_PEDIDO		NUM,"		
			cQuery += "			D2_ITEMPV		ITEM,"	
			cQuery += "			B1_COD			PRODUTO," 		
			cQuery += "			D2_LOTECTL		LOTE," 		
			cQuery += "			D2_DTVALID		DTVALID," 	
			cQuery += "			D2_QUANT		QTDLIB," 		
			cQuery += "			B1_RASTRO		RASTRO," 		
			cQuery += "			B1_DESC		DESCRIC," 		
			cQuery += "			B1_CODBAR		CODBAR,"
		 		
			cQuery += " 			ISNull( ( SELECT C6_QTDSEP "
			cQuery += " 						FROM " + RetSQLName("SC6")
			cQuery += " 						WHERE 	C6_FILIAL 	= '" + xFilial("SC6") + "'"
			cQuery += " 							AND	C6_NUM	  = D2_PEDIDO"
			cQuery += " 							AND	C6_ITEM   = D2_ITEMPV"
			cQuery += " 							AND	C6_ENVSEP = 1"
		...
		Else
			...
			cQuery := " SELECT 	C9_PEDIDO		NUM,"		
			cQuery += "			C9_ITEM		ITEM,"	
			cQuery += "			C9_PRODUTO		PRODUTO," 		
			cQuery += "			C9_LOTECTL		LOTE," 		
			cQuery += "			C9_DTVALID		DTVALID," 		
			cQuery += "			C9_QTDLIB		QTDLIB," 		
			cQuery += "			B1_RASTRO		RASTRO," 		
			cQuery += "			B1_DESC		DESCRIC," 		
			cQuery += "			B1_CODBAR		CODBAR,"
	...
	ElseIf cOrigem = "LOJ"
		...
		cQuery += "			L2_LOTECTL		LOTE,"	
		cQuery += "			L2_DTVALID		DTVALID,"	
		cQuery += "			L2_QUANT 		QTDLIB,"	
		cQuery += "			B1_RASTRO		RASTRO," 		
		cQuery += "			B1_DESC 		DESCRIC," 		
		cQuery += "			B1_CODBAR 		CODBAR,"
		
		cQuery += IIf( lLjQtVe, "L2_QUANT", "L2_QTDSEP" ) + " QTDSEP"
