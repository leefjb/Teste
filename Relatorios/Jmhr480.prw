	...
	If larquivo
		If ! Empty(cArquivo)
			...
			If FERROR() != 0
				...
				larquivo := .t.
				//Rafael Scheibler - Solutio IT
				//Cria o cabeçalho do CSV
				_cabec := "PRODUTO;DESCRICAO;ARMAZEM;FORNECEDOR;LOJA;DOCUMENTO;SERIE;EMISSAO;UNIDADE;QTD ORI;QTD SALDO;ITEM NF;PACIENTE;MÉDICO;DT CIRURGIA"
				fWrite( nHdl , _cabec + CRLF)
			Endif
		Else
			larquivo := .f.
		EndIf
		...
		While !Eof() .And. xFilial() == B6_FILIAL .And. cQuebra == B6_PRODUTO+B6_LOCAL
			...
			IF	IIf(B6_TPCF == "C" , &cCondCli , &cCondFor )
				...
				If !Empty(cProdLocal)
					...
					cTexto += If(mv_par15==1,cValtoChar(ConvUM(B6_PRODUTO,B6_QUANT,0,2)),cValtoChar(B6_QUANT));
					       + ' ; '

					// Saldo
					@ li,092+nIncCol PSay If(mv_par15==1,ConvUm(B6_PRODUTO,nSaldo,0,2),nSaldo) Picture PesqPict("SB6", "B6_QUANT",17)

					cTexto += If(mv_par15==1,cValtoChar(ConvUm(B6_PRODUTO,nSaldo,0,2)),cValtoChar(nSaldo));
					       + ' ; '
